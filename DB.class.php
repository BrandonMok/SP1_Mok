    ...
    class DB {
        ...
        function deleteAllUserInfo($data){
            try {
                ...
                if($deleteUSR > 0){
                    $managerEvent = $this->getAllManagerEvents(0, $data[0]);
                    if(count($managerEvent) > 0){
                        foreach($managerEvent as $mEvent){
                            $event = $this->getEvent($mEvent->getEvent());
                            $this->deleteEventAndSession($event->getIdEvent());
                        }
                    }
                }
            }   
